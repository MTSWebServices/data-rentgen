...
* Change response schema of `GET /operations`. ([#158](https://github.com/MTSWebServices/data-rentgen/issues/158))
    ...
    ??? note "Operations response examples"
        Before:
...
* Change response schema of `GET /runs`.  ([#159](https://github.com/MTSWebServices/data-rentgen/issues/159))
...
* Change response schema of `GET /locations`. ([#160](https://github.com/MTSWebServices/data-rentgen/issues/160))

    Location properties are moved to `data` key, added new `statistics` key.
    This allows to show location statistics in UI.

    ??? note "Locations response examples"
        Before:
    ...
    ??? note "Location response examples"
        Before:
...
* Change response schema of `GET /datasets`.  ([#161](https://github.com/MTSWebServices/data-rentgen/issues/161))
...
* Change response schema of `GET /jobs`.  ([#162](https://github.com/MTSWebServices/data-rentgen/issues/162))

    Job properties are moved to `data` key.
    This makes API response more consistent with others (e.g. `GET /runs`, `GET /operations`).

    ??? note "Jobs response examples"
        Before:
...
* Change response schema of `GET /:entity/lineage`. ([#164](https://github.com/MTSWebServices/data-rentgen/issues/164))
    ...
    ??? note "Lineage response examples"
        Before:
...
* Change `DATA_RENTGEN__KAFKA__BOOTSTRAP_SERVERS` value type from string (one `host:port` item) to list (`["host1:port1", "host2:port2"]`). ([#183](https://github.com/MTSWebServices/data-rentgen/issues/183))
* Sync `DATA_RENTGEN__KAFKA__SECURITY__TYPE` values with Kafka client values, e.g. `scram-sha256` -> `SCRAM-SHA-256`. ([#183](https://github.com/MTSWebServices/data-rentgen/issues/183))

## Features

* Consumer now can capture and save OpenLineage column lineage.  ([#155](https://github.com/MTSWebServices/data-rentgen/issues/155))
...
* Include sum inputs & outputs to lineage responses. ([#171](https://github.com/MTSWebServices/data-rentgen/issues/171))
...
* Add column lineage to lineage responses `GET /:entity/lineage`. ([#172](https://github.com/MTSWebServices/data-rentgen/issues/172))

    ??? note "Another lineage response examples"
        Before:
...
* Add support for Kafka GSSAPI auth. ([#183](https://github.com/MTSWebServices/data-rentgen/issues/183))
* Allow fetching `GET /v1/runs?since=...` without `search_query` query param. ([#184](https://github.com/MTSWebServices/data-rentgen/issues/184))

## Improvements

* Fix multiple `proxyUrl` for spark facet with `master="yarn"`. ([#154](https://github.com/MTSWebServices/data-rentgen/issues/154))
...
* Add dataset name parsing for removing partition-like part from name.  ([#175](https://github.com/MTSWebServices/data-rentgen/issues/175))
...
* Change logic for output/input dataset schema in lineage response. ([#185](https://github.com/MTSWebServices/data-rentgen/issues/185))
    ...
    ??? note "Dataset in lineage response examples"
        Before:
...
* Remove partition part of the name from dataset names. ([#188](https://github.com/MTSWebServices/data-rentgen/issues/188))
* Remove datasets and symlinks from lineage response which have no inputs or outputs. ([#189](https://github.com/MTSWebServices/data-rentgen/issues/189))
* Add `/v1/auth/logout` endpoint to KeycloakAuthProvider. ([#192](https://github.com/MTSWebServices/data-rentgen/issues/192))
