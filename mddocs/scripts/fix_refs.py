...
Replaces two kinds of unresolved cross-references:
  ...
  2. [text][data_rentgen.module.ClassName]  — Python-path refs that autorefs cannot
...
Options:
    ...
    --path PATH      Directory to process recursively (default: data_rentgen/).
...
#
# Used for [text][data_rentgen.module.ClassName] cross-references that autorefs
...
PYTHON_PATH_MAP: dict[str, str] = {
    # Add entries here for [text][data_rentgen.*] refs that autorefs cannot resolve
...
# class name (not a full data_rentgen.* path and not a kebab-case REF_MAP key).
# ---------------------------------------------------------------------------
...
#
# Used for [data_rentgen.path][] cross-references where the target is documented
...
#
# Used for [data_rentgen.path][] cross-references where the target is NOT documented
...
CODE_PATH_MAP: dict[str, str] = {
    ...
    # Used in a numpydoc "Returns" type position, so keep it a bare type name (no backticks).
    "data_rentgen.db.models.User": "User",
}
...
#
# Used for [text][data_rentgen.path] where the path is incorrect or the target
...
# ---------------------------------------------------------------------------
# Mapping: short [Name][] refs not in data_rentgen.* namespace
#
# Used for [Name][] where Name is a simple identifier or dotted non-data_rentgen path.
...
def _fix_refs(text: str) -> tuple[str, list[str]]:
    """Replace [label][] and [text][data_rentgen.*] with explicit Markdown links."""
    changes: list[str] = []
    ...
    def replace_python_path_self(m: re.Match) -> str:
        """Handle [data_rentgen.module.ClassName][] where the path is also the display text."""
    ...
    new_text = re.sub(r"\[([^\]]+)\]\[(data_rentgen\.[a-zA-Z0-9_.]+)\]", replace_python_path, new_text)
    new_text = re.sub(r"\[(data_rentgen\.[a-zA-Z0-9_.]+)\]\[\]", replace_python_path_self, new_text)
...
def main() -> None:
    ...
    parser.add_argument("--path", default="data_rentgen/", help="Directory to process (default: data_rentgen/)")
