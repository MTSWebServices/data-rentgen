...
This endpoint can be used retrieve job hierarchy graph (parents, dependencies) for a given job. (:issue:`407`, :issue:`412`)
...
Jobs can now reference a parent job via ``parent_job_id`` field. (:issue:`394`)
...
For example, it is possible to get Airflow DAG → Airflow Task → Spark app chain from a single response. (:issue:`392`, :issue:`399`, :issue:`401`)
...
This allows to show job type & name for specific run without sending additional requests. :issue:`411`
...
This allows to show last start time, status and duration for each job in the list, without additional requests. :issue:`387`
